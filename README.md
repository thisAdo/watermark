 ...
 * **Sin Dependencias del Sistema:** Incluye @ffmpeg-installer/ffmpeg para descargar y ejecutar FFmpeg sin requerir instalaciones manuales en tu computadora o servidor.
### Instalación
Para instalar el módulo directamente desde tu repositorio de GitHub usando la terminal (CLI), ejecuta el siguiente comando:
```bash
npm install github:thisAdo/watermark

```
Como este módulo utiliza la sintaxis moderna de módulos de ECMAScript (import/export), debes asegurarte de que el proyecto donde lo vayas a usar tenga configurado el "type": "module" en su archivo package.json. Tu package.json debería verse similar a esto:
```json
{
...
```
### Ejemplos de Uso
A continuación, se muestran los ejemplos de cómo utilizar el módulo en diferentes escenarios. En todos los casos, el módulo procesará los archivos y devolverá los enlaces finales.
...
async function aplicarMarcaDeAgua() {
    ...
    const result = await watermarkService.execute(
        '[https://cdn.adoolab.xyz/dl/8932d34a.jpg](https://cdn.adoolab.xyz/dl/8932d34a.jpg)',
        '[https://cdn.adoolab.xyz/dl/watermark.png](https://cdn.adoolab.xyz/dl/watermark.png)',
        'image',
        options
    );
...
async function aplicarMarcaDeAguaVideo() {
    ...
    const result = await watermarkService.execute(
        '[https://cdn.adoolab.xyz/dl/db6f3f3b.mp4](https://cdn.adoolab.xyz/dl/db6f3f3b.mp4)',
        '[https://cdn.adoolab.xyz/dl/watermark.png](https://cdn.adoolab.xyz/dl/watermark.png)',
        'video',
        options
    );
...
async function aplicarMarcaDeAguaAnimada() {
    ...
    const result = await watermarkService.execute(
        '[https://cdn.adoolab.xyz/dl/db6f3f3b.mp4](https://cdn.adoolab.xyz/dl/db6f3f3b.mp4)',
        '[https://cdn.adoolab.xyz/dl/e426a597.gif](https://cdn.adoolab.xyz/dl/e426a597.gif)',
        'video',
        options
    );
 ...
 * **watermark** *(String | Buffer)*: El archivo de la marca de agua. En videos, si esta marca de agua es animada, se recortará automáticamente a un máximo de 15 segundos para optimizar el procesamiento.
 * **type** *(String)*: El tipo de medio del archivo principal. Debe ser 'image' o 'video'. El valor por defecto es 'image'.
 * **options** *(Object)*: Objeto opcional de configuración para cambiar la apariencia y posición de la marca de agua.
**Objeto Options:**
| Propiedad | Tipo | Por defecto | Descripción |
|---|---|---|---|
| x | String | Number | 'right' | Posición horizontal. Acepta las posiciones 'left', 'center', 'right', o un número exacto en píxeles. |
| y | String | Number | 'bottom' | Posición vertical. Acepta las posiciones 'top', 'center', 'bottom', o un número exacto en píxeles. |
| margin | Number | 20 | Distancia en píxeles hacia los bordes (se aplica cuando usas textos como 'right' o 'bottom'). |
| width | Number | null | null | Fuerza a la marca de agua a cambiar su tamaño a este ancho específico en píxeles. La altura se ajusta sola. |
...
```javascript
{
  ...
  data: <Buffer>,
  urls: {
    ado: '[https://cdn.adoolab.xyz/dl/output.png](https://cdn.adoolab.xyz/dl/output.png)',
    maycol: '[https://cdn.soymaycol.icu/files/output.png](https://cdn.soymaycol.icu/files/output.png)'
  },
  cdnData: {
    ado: { ... },
    maycol: { ... }
 ...
 * **Zero System Dependencies:** Includes @ffmpeg-installer/ffmpeg to run seamlessly without requiring manual FFmpeg installations on your host machine.
### Installation
To install the module directly from the GitHub repository using the CLI, run the following command:
```bash
npm install github:thisAdo/watermark
 ...
 * **watermark** *(String | Buffer)*: The watermark file. In videos, if this is an animated watermark, it is automatically trimmed to 15 seconds maximum.
...
| margin | Number | 20 | Margin in pixels applied when using string relative positions. |
| width | Number | null | null | Forces the watermark to scale to a specific width in pixels, keeping the aspect ratio. |
