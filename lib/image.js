...
export async function processImageRemoval(source, region) {
    const key = 'TWlsaWstQm90LU91cmluTUQ=';
    const apiKey = Buffer.from(key, 'base64').toString('utf-8');
    ...
    const apiUrl = `https://api.neoxr.eu/api/photo-editor?image=${encodeURIComponent(imageUrl)}&apikey=${encodeURIComponent(apiKey)}`;
