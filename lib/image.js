...
export async function processImageRemoval(source, region) {
    ...
    let cropX = margin;
    if (region.x === 'right') cropX = sourceImg.width - w - margin;
    else if (region.x === 'center') cropX = (sourceImg.width - w) / 2;
    else if (typeof region.x === 'number') cropX = region.x;

    let cropY = margin;
    if (region.y === 'bottom') cropY = sourceImg.height - h - margin;
    else if (region.y === 'center') cropY = (sourceImg.height - h) / 2;
    else if (typeof region.y === 'number') cropY = region.y;

    ctx.filter = `blur(${region.blur || 15}px)`;
    ctx.drawImage(sourceImg, 0, 0, canvas.width, canvas.height);
    ctx.filter = 'none';

    const cleanPart = createCanvas(w, h);
    const cleanCtx = cleanPart.getContext('2d');
    cleanCtx.drawImage(sourceImg, cropX, cropY, w, h, 0, 0, w, h);

    ctx.drawImage(cleanPart, cropX, cropY);
