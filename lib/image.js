...
export async function processImageRemoval(source, region) {
    ...
    const getDimensions = (filePath) => new Promise((resolve, reject) => {
        ffmpeg.ffprobe(filePath, (err, metadata) => {
            ...
            if (stream) resolve({ width: stream.width, height: stream.height });
    ...
    return new Promise((resolve, reject) => {
        getDimensions(sourceFile.path).then(dimensions => {
            ...
            const filterComplex = `format=rgb24,split[base][tmp];[tmp]crop=${w}:${h}:${posX}:${posY},boxblur=${blur}:${blur}[blur];[base][blur]overlay=${posX}:${posY}:format=auto`;
