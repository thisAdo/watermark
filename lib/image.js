...
export async function processImageRemoval(source, region) {
    ...
    const cropX = region.x || 0;
    const cropY = region.y || 0;
    const cropW = region.width || 100;
    const cropH = region.height || 100;

    const cleanPart = createCanvas(cropW, cropH);
    const cleanCtx = cleanPart.getContext('2d');
    cleanCtx.drawImage(sourceImg, cropX, cropY, cropW, cropH, 0, 0, cropW, cropH);
