...
export async function processVideoRemoval(source, region) {
    ...
    const getDimensions = (filePath) => new Promise((resolve, reject) => {
        ffmpeg.ffprobe(filePath, (err, metadata) => {
            ...
            else reject(new Error('No se encontró stream de video'));
        });
    });
    ...
    return new Promise((resolve, reject) => {
        ...
        getDimensions(sourceFile.path).then(dimensions => {
            ...
            const filterComplex = `split=2[base][tmp];[tmp]crop=${w}:${h}:${posX}:${posY},boxblur=${blur}:${blur}[blurred];[base][blurred]overlay=${posX}:${posY}`;
            ...
            reject(new Error(`Error al leer dimensiones del video: ${err.message}`));
        });
    });
}